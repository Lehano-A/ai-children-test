...
const dataResult: Data = {
  title: '**Психологический отчёт о ребёнке 8 лет**',
...
export const fetchResultProcessing = createAsyncThunk(
  `fetch/${RESULT_PROCESSING}`,
  // api request
  async (_task_id: string, { dispatch, signal }) => {
    dispatch(setLoading({ componentName: RESULT_PROCESSING, status: true }))
    ...
    return new Promise<PromiseResult>((resolve) => {
      const interval = setInterval(() => {
        ...
        if (randomNum === 1) {
          ...
          resolve({ status: 'done', data: dataResult })
        }
      }, 4440)
